...
class GradCAM:
    ...
    def __call__(
        ...
        if index is None:
            index = int(np.argmax(outputs.detach().cpu().numpy()))
...
def apply_gradcam_on_image(img: np.ndarray, mask: np.ndarray):
    ...
    heatmap_ = np.float32(heatmap) / 255
    cam = heatmap_ + np.float32(img)  # (H, W, 3)
    cam = cam / np.max(cam)
