...
def lovasz_softmax(
    ...
    Multi-class Lovasz-Softmax loss
      probas: [B, C, H, W] Variable, class probabilities at each prediction
              (between 0 and 1). Interpreted as binary (sigmoid) output with
              outputs of size [B, H, W].
...
def mean(lis, ignore_nan=False, empty=0):
    """
    nanmean compatible with generators.
    """
    lis = iter(lis)
    if ignore_nan:
        lis = filterfalse(isnan, lis)
    try:
        n = 1
        acc = next(lis)
